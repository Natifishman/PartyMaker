...
public class AdminSettingsActivity extends AppCompatActivity {
    ...
    if (resultCode == Activity.RESULT_OK) {
      if (requestCode == 100) {
        ...
        if (null != uri) {
          ...
          StorageReference storageRef = DBRef.refStorage.child("Groups/" + GroupKey);
          ...
          view -> {
            ...
            } else if (finalI == 1) // open 1,2 (2) set if anyone can add/only admin can add
            {
              ...
              if (CanAdd) {
                ...
                CanAdd = false;
                DBRef.setValue(DBRef.refGroups.child(GroupKey).child("canAdd"), false)
                    .addOnFailureListener(e -> {
                      Log.e("AdminSettings", "Failed to update canAdd", e);
                      Toast.makeText(AdminSettingsActivity.this, "Failed to update settings", Toast.LENGTH_SHORT).show();
                    });
              } else {
                imgCanAdd.setImageResource(R.drawable.ic_party_everyone);
                tvCanAdd.setText("Everyone Add");
                CanAdd = true;
                DBRef.setValue(DBRef.refGroups.child(GroupKey).child("canAdd"), true)
                    .addOnFailureListener(e -> {
                      Log.e("AdminSettings", "Failed to update canAdd", e);
                      Toast.makeText(AdminSettingsActivity.this, "Failed to update settings", Toast.LENGTH_SHORT).show();
                    });
            ...
            } else if (finalI == 2) // open 2,1 (3) Delete Group
            {
              ...
              DBRef.removeValue(DBRef.refGroups.child(GroupKey))
                  ...
                  .addOnFailureListener(e -> {
                    Log.e("AdminSettings", "Failed to delete group", e);
                    Toast.makeText(AdminSettingsActivity.this, "Failed to delete group", Toast.LENGTH_SHORT).show();
                  });
  ...
  private void deleteMessages() {
    ...
    if (MessageKeys == null) {
      ...
      android.util.Log.w("AdminSettingsActivity", "MessageKeys was null in deleteMessages() method, initialized with empty HashMap");
