...
public class GroupDetailsActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    // Initialize HashMaps if they are null
    if (FriendKeys == null) {
      FriendKeys = new HashMap<>();
      Log.w(TAG, "FriendKeys was null, initialized with empty HashMap");
    }
    if (ComingKeys == null) {
    ...
    if (MessageKeys == null) {
      ...
      Log.w(TAG, "MessageKeys was null, initialized with empty HashMap");
    }
            ...
            } else if (finalI == 7) // open 4,2 (8) Leave
            {
              ...
              builder.setPositiveButton(
                  ...
                  (dialogInterface, i2) -> {
                    ...
                    else if (FriendKeys != null && FriendKeys.size() > 1) {
  ...
  private void isComing() {
    ...
    if (ComingKeys == null) {
      ComingKeys = new HashMap<>();
      Log.w(TAG, "ComingKeys was null in isComing() method, initialized with empty HashMap");
    }
  ...
  private void deleteMessages() {
    ...
    if (MessageKeys == null) {
      MessageKeys = new HashMap<>();
      Log.w(TAG, "MessageKeys was null in deleteMessages() method, initialized with empty HashMap");
    ...
    DBRef.refMessages.addValueEventListener(
        new ValueEventListener() {
            ...
            for (DataSnapshot data : dataSnapshot.getChildren()) {
              ...
              for (String chatMsgKey : MessageKeys.keySet()) {
                if (chatMsgKey.equals(msgKey)) {
                  // מחיקה דרך השרת
                  DBRef.removeValue(DBRef.refMessages.child(chatMsgKey))
                      .addOnFailureListener(e -> 
                          Log.e(TAG, "Failed to delete message: " + chatMsgKey, e));
                }
              }
            }
          }
          ...
          public void onCancelled(@NonNull DatabaseError databaseError) {
            Log.e(TAG, "Database error when deleting messages", databaseError.toException());
          }
        });
  }
}
