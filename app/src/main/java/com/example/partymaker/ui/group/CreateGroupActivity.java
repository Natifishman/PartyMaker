...
public class CreateGroupActivity extends AppCompatActivity implements OnMapReadyCallback {
  ...
  private static final int action_create_group = R.id.action_create_group;
  private static final int action_edit_profile = R.id.action_edit_profile;
  private static final int action_public_groups = R.id.action_public_groups;
  private static final int action_refresh = R.id.action_refresh;
  ...
  private void initializeDatabase() {
    try {
      ...
      Log.d(TAG, "Database service initialized successfully");
    } catch (Exception e) {
      Log.e(TAG, "Error initializing database service", e);
      showErrorMessage("Database initialization failed");
    }
  }
    ...
    if (!Places.isInitialized()) {
      try {
        ...
        if (apiKey == null || apiKey.isEmpty()) {
          Log.e(TAG, "Google Maps API Key is empty. Please add a valid key to assets/local.properties");
          Toast.makeText(this, "Missing Google Maps API Key", Toast.LENGTH_LONG).show();
          finish(); // Close the activity since we can't proceed without the API key
          return;
        }
        Places.initialize(getApplicationContext(), apiKey);
        Log.d(TAG, "Google Places API initialized successfully");
      } catch (Exception e) {
        Log.e(TAG, "Failed to initialize Places API", e);
        Toast.makeText(this, "Error initializing Google Maps", Toast.LENGTH_LONG).show();
  ...
  private Group createGroupFromForm() {
    ...
    group.setGroupName(etPartyName.getText().toString());
    group.setAdminKey(getCurrentUserEmail());
    group.setGroupType(determineGroupType());
    group.setCreatedAt(getCurrentTimestamp());
    group.setGroupPrice("0");
    group.setGroupLocation(getSelectedLocation());
    
    // Set date and time
    GroupDateTime dateTime = getSelectedDateTime();
  ...
  private void saveGroupToDatabase(Group group) {
    ...
    // Save group to database using server
    CompletableFuture<ServerResponse<Object>> future = databaseService.setValue(GROUPS_DB_PATH + "/" + groupKey, group);
    future.thenAccept(response -> {
      if (response != null && response.isSuccess()) {
        ...
        Log.e(TAG, "Error saving group to database: " + (response != null ? response.getMessage() : "Unknown error"));
        showErrorMessage("Failed to save group");
      }
    }).exceptionally(e -> {
      Log.e(TAG, "Error saving group to database", e);
      showErrorMessage("Failed to save group: " + e.getMessage());
      return null;
    });
  }
  ...
  private void addAdminToGroup(Group group) {
    ...
    // Add to both friend keys and coming keys using server
    String friendKeysPath = GROUPS_DB_PATH + "/" + group.getGroupKey() + "/FriendKeys";
    String comingKeysPath = GROUPS_DB_PATH + "/" + group.getGroupKey() + "/ComingKeys";
    
    databaseService.updateChildren(friendKeysPath, memberData);
    databaseService.updateChildren(comingKeysPath, memberData);
  }

  private void initializeGroupChat(Group group) {
    Map<String, Object> emptyChat = new HashMap<>();
    String messageKeysPath = GROUPS_DB_PATH + "/" + group.getGroupKey() + "/MessageKeys";
    
    databaseService.updateChildren(messageKeysPath, emptyChat);
  }
    ...
    if (resultCode == Activity.RESULT_OK) {
      if (requestCode == 100) {
        ...
        if (null != uri) {
          ...
          StorageReference storageRef = DBRef.refStorage.child("Groups/" + GroupKey1);
          DBRef.putFile(storageRef, uri)
